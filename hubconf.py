...
def create(name, pretrained, channels, classes, autoshape):
    ...
    try:
        ...
        model = YoloBody(cfg, channels, classes)
        if pretrained:
...
def custom(path_or_model='path/to/model.pt', autoshape=True):
    ...
    hub_model = YoloBody(model.yaml).to(next(model.parameters()).device)  # create
...
if __name__ == '__main__':
    model = custom(path_or_model='logs/best_epoch_weights.pth')  # custom example
